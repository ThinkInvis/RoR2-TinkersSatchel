...
namespace ThinkInvisible.TinkersSatchel {
    public class Moustache : Item<Moustache> {
        ...
        [AutoConfig("Percent chance to trigger this item's effect on hit, per stack (hyperbolic).", AutoConfigFlags.None, 0f, 100f)]
        public float procChance { get; private set; } = 4f;

        [AutoConfigRoOSlider("{0:N1} s", 0f, 30f)]
        [AutoConfigUpdateActions(AutoConfigUpdateActionTypes.InvalidateLanguage)]
        [AutoConfig("Fixed taunt duration.", AutoConfigFlags.None, 0f, float.MaxValue)]
        public float procDuration { get; private set; } = 5f;

        [AutoConfigRoOSlider("{0:P0}", 0f, 1f)]
        [AutoConfigUpdateActions(AutoConfigUpdateActionTypes.InvalidateLanguage | AutoConfigUpdateActionTypes.InvalidateStats)]
        [AutoConfig("Fixed damage resistance vs. taunted enemies.", AutoConfigFlags.None, 0f, 1f)]
        public float damageResist { get; private set; } = 0.15f;

        [AutoConfigRoOSlider("{0:P0}", 0f, 1f)]
        [AutoConfigUpdateActions(AutoConfigUpdateActionTypes.InvalidateLanguage | AutoConfigUpdateActionTypes.InvalidateStats)]
        [AutoConfig("Fixed knockback resistance vs. taunted enemies.", AutoConfigFlags.None, 0f, 1f)]
        ...
        private void SetStateOnHurt_OnTakeDamageServer(On.RoR2.SetStateOnHurt.orig_OnTakeDamageServer orig, SetStateOnHurt self, DamageReport damageReport) {
            ...
            var count = GetCountEffective(damageReport.attackerMaster);
