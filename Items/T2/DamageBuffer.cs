...
namespace ThinkInvisible.TinkersSatchel {
    public class DamageBuffer : Item<DamageBuffer> {
        ...
        protected override string GetDescString(string langid = null) => $"<style=cIsDamage>{Pct(bufferFrac)} <style=cStack>(+{Pct(bufferFrac)} per stack, inverse-mult.)</style> of incoming damage</style> is <style=cIsHealing>applied gradually</style> over {bufferDuration}, ticking every {bufferRate} seconds. <style=cIsHealing>Healing</style> past <style=cHealth>max health</style> <style=cIsHealing>will apply</style> to the pool of delayed damage.";
